...
static int SplitPage(SceUIDMemBlockObject *pMemBlock, SceKernelMemBlockPage *pPage, void *targetAddr, SceSize targetSize)
{
    ...
    pNewPage->flags = (pPage->flags & ~0x3F) | 0x10; // 0x10 is used for pages created with a specific physical address (CDRAM pages also).

    if (pPage->vaddr < targetAddr)
        pNewPage->size = pPage->size - (targetAddr - pPage->vaddr); // New page will inhabit the area below the target
    else
...
int kuKernelMemProtect(void *addr, SceSize size, SceUInt32 prot)
{
    ...
    if (size & 0xFFF)
    {
        LOG("Error: Size must be page-aligned (0x1000 bytes)");
        return SCE_KERNEL_ERROR_INVALID_ARGUMENT;
    }
    ...
    while (curSize)
    {
        ...
        checkPage:
            ...
            {
                if ((ret = SplitPage(pMemBlock, pPage, curAddr, curSize)) < 0)
                    break;
                goto checkPage;
            }
